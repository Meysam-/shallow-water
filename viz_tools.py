...
def eta_animation3D(X, Y, eta_list, bed, frame_interval, filename):
    fig = plt.figure(figsize=(8, 8), facecolor="white")
    ax = fig.add_subplot(111, projection='3d')

    surf = ax.plot_surface(X, Y, eta_list[0], cmap=plt.cm.RdBu_r)
    surf = ax.plot_surface(X, Y, bed, cmap=plt.cm.jet)

    def update_surf(num):
        ax.clear()
        surf = ax.plot_surface(X / 1000, Y / 1000, eta_list[num], cmap=plt.cm.RdBu_r)
        ax.plot_surface(X / 1000, Y / 1000, bed, cmap=plt.cm.jet)
